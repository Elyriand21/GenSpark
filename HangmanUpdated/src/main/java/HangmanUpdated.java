...
public class HangmanUpdated {
    ...
    private void displayGameInterface() {
        System.out.println("ScoresPath Abso: " + scoresPath);
        writeToFile();
        readArtFromFile();
    }
    ...
    private void evaluate() {
        ...
        // Letter has not been guessed yet
            ...
            correctLetter(ch);
            System.out.println("ContainsCharacter: " + containsCharacter);
            ...
            if (!containsCharacter) {
                System.out.println("Got inside !containsCharacter");
                missedLetters += ch;
        ...
        if (lettersLeft == 0) {
            System.out.println("Got into lettersLeft = 0");
            state = 7;
            writeToFile();
            wordGuessed = true;
        }
        if (guesses == 7) {
            System.out.println("Got into guesses = 7");
            wordGuessed = true;
        }
        indx = 0;
    }
    ...
    private void writeToFile() {
        ...
        // 1 = after creation
            ...
            try {
                ...
                blankFiltered.stream().forEach((s) -> {
                    try {
                        fw.write(s + "\n");
                    } catch (IOException e) {
                        e.printStackTrace();
        ...
        // 2 = Updating high score
            ...
            try {
                ...
                scores1.stream().forEach((s) -> {
                    try {
                        fw.write(s + "\n");
                        System.out.println("Successfully wrote to scores.txt");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                });
        ...
        try (Stream<String> stream = Files.lines(scoresPath)) {
            ...
            highScores.stream().forEach((s) -> {
                if (s.startsWith("x")) {
                    ...
                    System.out.println(Integer.parseInt(s.substring(17)));
